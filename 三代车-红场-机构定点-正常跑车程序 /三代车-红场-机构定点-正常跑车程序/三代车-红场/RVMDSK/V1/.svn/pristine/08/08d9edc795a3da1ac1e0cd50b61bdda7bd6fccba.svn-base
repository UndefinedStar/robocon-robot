...
void LCD_CtrlLinesConfig(void)
{
  ...
  /* Enable AFIO GPIOD, GPIOE, GPIOF, GPIOG clocks */
  ...
  GPIO_Init(GPIOE, &GPIO_InitStructure);
	 /* Set PE.4(A20 (RS)) as alternate function push pull */
...
// second release on 3/5  ,luminance is acceptable,water wave appear during camera preview
  ...
  //Panel Interface Control//
	...
	else if(DeviceCode==0x8989)											//2.8寸屏是8989的ID号
	{
	    ...
	    LCD_WriteReg(0x0010,0x0000);     
	    LCD_WriteReg(0x0011,0x6078);		//0x4030	//定义数据格式  16位色	横屏 0x6058  竖屏6070		//这个寄存器很重要
