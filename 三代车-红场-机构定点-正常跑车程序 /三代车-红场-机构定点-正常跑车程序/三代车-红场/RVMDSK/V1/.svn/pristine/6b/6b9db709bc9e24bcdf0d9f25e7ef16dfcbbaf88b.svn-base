...
void OV_DCMI_DMA_Init(uint32_t address, uint16_t buffer_size)
{
  ...
  nvic.NVIC_IRQChannelSubPriority = 3;		//图像的优先级一定要最高，比如CAN中断的优先级如果高于图像采集优先级，则会导致
  nvic.NVIC_IRQChannelCmd = ENABLE;				//图像采集出错
  NVIC_Init(&nvic);
