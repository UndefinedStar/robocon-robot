...
// Í¼Ïñ²ÎÊı
int sp_threshold_value = 15,hd_threshold_value = 15,xp_threshold_value = 3;		// Ìø±äãĞÖµ
int sp_value_sub = 15,hd_value_sub = 15,xp_value_sub = 5;			// ¶şÖµ»¯ãĞÖµ

// ÉÏÆÂ²ÎÊı
int ZjClearDistance = 200,ZjDetectDistance = 210;	// 219
int ZjBigAngleRow = 15;
int ZjScSpeed = 6;

// ºÓµÀ²ÎÊı
int Hd1ToHd5Distance = 66,Hd1ToXpDistance = 105;  // 61 87
float race_hd1 = 0.4,race_hd2 = -0.12,race_hd3 = 0.2,race_hd4 = -0.2,race_hd5 = 0.8;
float HdAngle = 1.7;
int HdScSpeed = 8;

// ÏÂÆÂ²ÎÊı
int XpScDistance = 45,HuorCross = 75;	//74.5
float Xp3RaceTop = 0.6,Xp3RaceCross = 0.5;
float xpp1 = 0.38,xpp2 = 0.43,xpp3 = 0.5;
int XpScSpeed = 9;



// ÆäËû²ÎÊı
int left[R]={0};					// ¼ÇÂ¼×ó±ßÔµµÄÊı×é8
int center[R+1]={0};
int right[R]={0};					// ¼ÇÂ¼ÓÒ±ßÔµÁĞµÄÊı×é
int number = 2;						// ´Ó×îºóÒ»ĞĞ²É¼¯µ½µ¹ÊıµÚÈıĞĞ£¬ºöÂÔ×îÔ¶Á½ĞĞ
int left_angle_flag = 0, right_angle_flag = 0,rr=0;
...
int zj_time = 0,hand_time = 0,time_axial = 0,zj_end_time = 0;
int correct_row = 0;
u8 UVFlag = 0;
...
void image_threshold_sp(u8 *Data)
{
	...
	for(s=0;s<10;s++)
	{
		...
		max_value = 0;
		max_col = 0;
		/* ±éÀúÒ»ĞĞÑ°ÕÒÏñËØÌø±ä²îÖµ×î´óµÄµã£¬×÷Îª±ßÔµ¼ÇÂ¼ */     //R 41   C 80
		for (a =0; a < C-3; a++)
		{
	...
	if(((double)white_count_ratio > 0.5||(double)white_count_ratio < 0.01)||(Detect>=1&&(double)white_count_ratio>0.32))
		whitelost_left = 1; 
...
void image_threshold_hd(u8* Data)
{
		...
		if(white_count_ratio>0.2||white_count_ratio<0.015)
			whitelost_left = 1;
}
...
//			min = min2;
	...
	if(((double)white_count_ratio < 0.01||(double)white_count_ratio>0.12))
		whitelost_left = 1; 	
...
{ 
	if(whitelost_left == 0)
	{
		...
		for(m=R-1;m>2;m--)
		{
			...
			else if(m<R-1)
			{
				...
				if (right_number > 4&&right_angle_flag == 0&&nu<38)
				{
					zj_detect = 1;
					right_angle_flag = 1;
					if(sp_flag==1&&right_angle_flag==1)
						zj_start=1;				
					rr = m;
				}
		...
		for(m=R-15;m>2;m--)
		{
			if(row_white[m]>7&&fabs(row_white[m]-row_white[m+2])>4)
			{
				right_angle_flag=1;
				zj_detect=3;
			}
...
void race_identity()
{
	...
	if((hd_flag==1&&hd_stage_flag==5&&fabs(race)<0.15&&fabs(race)>0.02))
	{
		xp_flag=1;
		hd_flag=0;
		stop_time = 0;
...
float error_ca()
{
	...
	if(nu<38&&right_angle_flag==0&&sp_flag==1&&HuorDistance>ZjDetectDistance)
		right_angle_flag = 1;
	...
	if((sp_flag==1&&right_angle_flag==1&&Detect>=1))
	{
		if(whitelost_left==0)
		{
			if(nu>=25&&white_count_ratio>0.02&&white_count_ratio<0.32)
			{
				for(rw=R-1;rw>R-10;rw--)
				{
						...
						if(rw_white_count>3&&rw_white_count<10&&fabs(center[rw]-center[rw-1])<3)
							rw_effec++;
				}
				if(rw_effec<8)
					whitelost_left = 1;
  ...
  if(whitelost_left==0)
	{
		...
		if(img_stop_flag==3&&fabs(race)>Xp3RaceTop&&nu>30&&img_cross_flag==0)
			img_cross_flag = 1;
		if(img_stop_flag==3&&fabs(race)<Xp3RaceCross&&fabs(race)>0.15&&nu>30&&img_cross_flag==1)
			img_cross_flag = 2;
		if(img_stop_flag>=3&&HuorDistance-speed_stop2>HuorCross&&fabs(race)<0.5&&speed_stop2>0)
			img_cross_flag = 2;
		if(img_cross_flag==2)
		{
			if(img_stop_flag==3&&(nu<35||fabs(race)>0.6||(nu==35&&fabs(race)<=0.52&&center_stop_max-center_stop_min>34)))
				whitelost_left = 1;
			else if(img_stop_flag==4&&(fabs(race)>0.3||nu<30||(nu>30&&fabs(race)<=0.25&&center_stop_max-center_stop_min>22)))
				whitelost_left = 1;
			if(white_count_ratio>0.08||white_count_ratio<=0.02)
				whitelost_left = 1;
		}
	}
	// ¼ÇÂ¼»ô¶û´«¸ĞÆ÷Öµ
	if(speed_left==0&&sp_flag==1&&right_angle_flag==1)  
	...
	else                        //Í¼ÏñÕı³£
	{
		...
		{
			right_angle_flag = 0;
			if(((double)white_count_ratio<0.3&&(double)white_count_ratio>0.01&&nu>27&&HuorDistance-speed_left>15&&speed_left>0))  //ÕâÀïĞèÒª¸ü¼ÓÑÏ¸ñ
			{ 
				sp_flag=0;
				hd_flag=1;
				late_cut_down = 0;
		...
		{		
			// xiugai-6
			if((double)race>race_hd1&&hd_stage_flag==0&&nu>36)  //ÒÔºóÒªÏ¸µ÷Ğ±ÂÊ,ÔÚ¹Ì¶¨ÉãÏñÍ·µÄÇé¿öÏÂ,¸ù¾İ×ß³¡²ÎÊı¼ÈÊÇÈ·¶¨£¬²âÊÔÓÃ
			{
				hd_stage_flag = 1;			
			}
			else if(nu>30&&(double)race<race_hd2&&hd_stage_flag==1)//1,²âÊÔÓÃ
			{
				hd_stage_flag = 2;
			}
			...
			else if((((double)race>race_hd5)&&hd_stage_flag==4))
			{
				hd_stage_flag = 5;
			}
		}
	...
	if(xp_flag==1)
	{
		if((double)race>0.3&&img_stop_flag==0&&((speed_hd1>0&&HuorDistance-speed_hd1>120)||XpHuorStartFlag==1))
		{
			img_stop_flag=1;
		}
		else if((double)race<-0.3&&img_stop_flag==1&&((speed_hd1>0&&HuorDistance-speed_hd1>140)||XpHuorStartFlag==1))
		{
			img_stop_flag=2;
			stop_time = 0;
		}
		else if((double)race>0.4&&img_stop_flag==2)
		{
			img_stop_flag=3;
		} 
	...
	if(img_cross_flag>=2)
	{
		if(number<16)
		{
			...
			if(fabs(control_message-control_message2)>5)
				control_message = control_message2;
		}
		else
			control_message = control_message2;
	...
	/* ÉÏÆÂ¹ıÖ±½Ç£¬·ÖÁ½¸ö½×¶Î */
	if(sp_flag==1&&HuorDistance>ZjDetectDistance&&control_message<-5)
		control_message = 0;
	...
	if(sp_flag==1&&right_angle_flag==1)
	{
		...
		if(zj_speed<5)
			...
			small_angle_row = ZjBigAngleRow+4;
		else
			small_angle_row = ZjBigAngleRow+5;
	...
	{
		if(fabs(control_message-control_message2)<15)
			control_message2 = control_message;
	}
