    ...
    switch (KeyNumber & 0x07)
    {
        ...
        case 0x03:
            ...
            while(GPIO_ReadInputDataBit(GPIOF, GPIO_Pin_11) == StartKey)
                OSTimeDly(100);
            Cylinder(CylinderSetHeart | CylinderHand | Cylinderbuffer |
                     CylinderClimbOneMotor,
                     CylinderHandOpen | CylinderSetHeartOpen | CylinderbufferClose |
                     CylinderClimbOneClose, 'W');
            MotorInit();
    ...
    while(1)
    {
        ...
        switch (KeyNumber & 0x07)
        {
            case 0x00:
                ...
                Led8DisData(2);
                RunPoint(1, 1, 0, 0, 200, 250, 50, PositionNow.angle, 338 + CarBehindPoint, 50,
                         0, 140, 140, 0, 0);
                Led8DisData(3);
...
//#endif
                ...
                RunPoint(1, 0, 1, 1, 380 - Switch_SpeedDown * 100,
                         Vbegin - Switch_SpeedDown * 50, 380 - Switch_SpeedDown * 100, 10,
                         260 + PathPoint, 280, 0, Abegin, 0, 0, 200);
//                RunPoint(1, 0, 1, 1, 380, 120, 380, 15, 260 + PathPoint, 280, 0,180, 0, 0, 200);
            ...
            case 0x02:
                ...
                Vmax = 400;
                Led8DisData(2);
                RunPoint(1, 1, 0, 0, 200, 250, 50, PositionNow.angle, 345 + CarBehindPoint, 50,
                         0, 140, 140, 0, 0);
                Led8DisData(3);
                Flag[StartDt50Work] = 1;
                Flag[AutoRun] = 1;
                ...
                Flag[FanSpeedChangeByDT50] = 0;
                Flag[HeightChangeByCar] = 1;
                RunPoint(1, 0, 0, 1, 250, 50, 250, PositionNow.angle, 260 + PathPoint, 280, 0,
                         170, 0, 0, 0);
                Led8DisData(4);
            ...
            case 0x03:
                ...
                OSSemPost(ArmStart);
								MotorSearchZero(MotorHeartHand, 3000*PlacePart);
                RunPoint(1, 1, 1, 1, 250, 200, 150, PositionNow.angle, 55, 650, -90, 100, 100,
                         50, 200);
                Flag[SpeedChange] = 1;
                Flag[StartDt50Work] = 1;
                Vmax = 30;
                RunPoint(0, 0, 0, 1, 320, 320, 320, PositionNow.angle, 47 + AnglePoint, 870,
                         -90, 0, 0, 0,
                         0);
                if(Flag[FanStop] == 0)
                {
            ...
            case 0x04:
                ...
                OSSemPost(ArmStart);
                StartFan(600);
                RunPoint(1, 1, 0, 1, 300, 100, 200, PositionNow.angle, 55, 420, 0, 200, 100, 50,
                         250);

                StartFan(800);
                RunPoint(0, 0, 1, 1, 200, 200, 200, PositionNow.angle, 55, 830, -100, 100, 70,
                         80, 280);
                StartFan(0);
                OSTaskSuspend(25);
                MotorChangeMode(MotorFanCircle, ModePosition);
            ...
            case 0x05:
                ...
                MotorPosition(MotorFanCircle, 4000, -105 * (330)*PlacePart);
#endif
                SetBezier(PositionNow.x, PositionNow.y, 430, 1316 + LineY, 90, 0, 30, 300);
                RunBezier(1, 1, 0, 1, 250, 200, 250, -180, 0.3, 0.5, 0, 0.2, BPidGROUP3);
...
//              Cylinder(CylinderHand, CylinderHandClose, 'W');
        ...
        MotorPosition(MotorHeartHand, 9000, -37000);
        while(MotorRealPosition.HeartHand < -37500)
            OSTimeDly(100);
#else
//        StartStteringEngine(StteringEngineBlue, 1500);
        ...
        if((KeyNumber & 0x07) != 6 || (KeyNumber & 0x07) != 7)
        {
            ...
            for(i = 0; i < 50; i++)
            {
                StartStteringEngine(StteringEngineBlue, 2030 - i * 7.5);
                OSTimeDly(100);
            }
...
//          NVIC_SystemReset();
        ...
        OSTimeDly(2000);
        while(sqrt(SpeedAbsolute.Vx*SpeedAbsolute.Vx+SpeedAbsolute.Vy*SpeedAbsolute.Vy) < 10)
        {
            Elmo_Ele();
            FREE_ALL_MOTOR;
            BeepOn;
