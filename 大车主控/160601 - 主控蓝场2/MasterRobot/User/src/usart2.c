...
void USART2_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) //接收寄存器非空
    {
        ...
        {
            if(USART2_RX_BUF[0]=='R' && USART2_RX_BUF[1]=='C')
			{
				UsartErrorTme=0;
				DecodeS16Data(&ErrorCameraWidth,&USART2_RX_BUF[2]);
				DecodeS16Data(&ErrorCameraHeight,&USART2_RX_BUF[4]);

				if(USART2_RX_BUF[4]==0)
					Flag[LostPicture]=0;
				else if(USART2_RX_BUF[4]==1)
					Flag[LostPicture]++;
			}
			if(USART2_RX_BUF[0]=='S' && USART2_RX_BUF[1]=='T')
			{
