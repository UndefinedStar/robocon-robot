...
// Í¼Ïñ²ÎÊı
int sp_threshold_value = 10,hd_threshold_value = 10,xp_threshold_value = 6;		// Ìø±äãĞÖµ,15,15,5
int sp_value_sub = 15,hd_value_sub = 15,xp_value_sub = 10;			// ¶şÖµ»¯ãĞÖµ	15,15,10

// ÉÏÆÂ²ÎÊı
int ZjClearDistance = 195,ZjDetectDistance = 205;	// 211
int ZjBigAngleRow = 23;
int zj_time = 0;

// ºÓµÀ²ÎÊı
int Hd1ToHd5Distance = 70,Hd1ToXpDistance = 108;  // 64 95
float race_hd1 = -0.4,race_hd2 = 0.12,race_hd3 = -0.2,race_hd4 = 0.2,race_hd5 = -0.7;

// ÏÂÆÂ²ÎÊı
int XpScDistance = 45,HuorCross = 75;	//74.5
float Xp3RaceTop = 0.6,Xp3RaceCross = 0.4;
...
void Transfer2Gray_UV(uint16_t *image,uint8_t  *gray_image)
{
	...
	{	
		...
		for(j=0;j<C;j++)
		{			
			*((uint8_t *)gray_image+i*C+j)=(uint8_t)((*((uint16_t *)image+(i*(4+a)-a*(a+1)/2*15+3)*IMAGE_COLUMN+j*4)>>8));// Ç°15£¬Ã¿4ĞĞÈ¡Ò»ĞĞ£¬ÖĞ¼äÃ¿5ĞĞÈ¡Ò»ĞĞ£¬ºóÃæÃ¿6ĞĞÈ¡Ò»Ğ
		}
	}			
}
...
//			min = min2;
		...
		if(img_cross_flag==2)
		{
			...
			if (c > 2 && c < R - 3)
			{
					if ((row_white_count[c] > 8 && row_white_count[c + 1] <= 8&&com_count<6))
					{
							if(cross_flag==0)
								cross_flag = c;
...
//		left_angle_flag = 1;
	...
	if((sp_flag==1&&left_angle_flag==1&&Detect>=1))
	{
		if(whitelost_left==0)
		{
			if(nu>=15&&white_count_ratio>0.02&&white_count_ratio<0.25)
			{
				for(rw=R-1;rw>R-10;rw--)
				{
						rw_white_count = 0;
						for(cw=center[R-1]-6;cw<center[rw]+6;cw++)
						{
							if(cw>=0&&cw<C)
							{
  ...
  if(whitelost_left==0)
	{
		...
		if(img_stop_flag==3&&fabs(race)>0.6&&nu>30&&img_cross_flag==0)
			img_cross_flag = 1;
		else if(img_stop_flag==3&&fabs(race)<0.4&&fabs(race)>0.15&&nu>30&&img_cross_flag==1)
			img_cross_flag = 2;
		...
		if(img_cross_flag==2)
		{
			if(img_stop_flag==3 && 
				(nu<35 || 
			   fabs(race)>=0.5 || 
			   (center_stop_max-center_stop_min>20) ||
			    race<-0.15))
				whitelost_left = 1;
			else if(img_stop_flag==4&&(nu<37||fabs(race)>=0.25||center_stop_max-center_stop_min>15))
				whitelost_left = 1;
			if(white_count_ratio>0.062||white_count_ratio<0.03)
				whitelost_left = 1;
		}	
	}
	// ¼ÇÂ¼»ô¶û´«¸ĞÆ÷Öµ
	if(speed_left==0&&sp_flag==1&&left_angle_flag==1)  
	...
	else                        //Í¼ÏñÕı³£
	{
		...
		{
			if(((double)white_count_ratio<0.15&&(double)white_count_ratio>0.01&&nu>30&&zj_time>3&&HuorDistance-speed_left>20&&speed_left>0))  //ÕâÀïĞèÒª¸ü¼ÓÑÏ¸ñ
			{ 
				sp_flag=0;
				hd_flag=1;
				late_cut_down = 0;
...
//	}
	if(sp_flag==1&&left_angle_flag==1)
	{
		...
		if(nu<ZjBigAngleRow+18&&speed_now>=6)
			late_cut_down = 1;
		else
			late_cut_down = 0;
...
void Screen(void)
{
	...
	if(hd_flag==1)
	{
		...
		number_show(60,120,speed_hd1,3);
	}
	else if(xp_flag==1)
	{ 
	...
	number_show(70,180,control_message,3);
}
