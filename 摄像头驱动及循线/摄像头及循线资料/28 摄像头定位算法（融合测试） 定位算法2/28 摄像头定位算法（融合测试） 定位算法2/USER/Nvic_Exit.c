...
调用形式:
Exit_Init(GPIOA, GPIO_Pin_3, GPIO_Mode_IPU, EXTI_Trigger_Falling, 2, 0);
*************************************************/
			   ...
			   u8 sub_pri)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannel = Exit_IRQn[gpio_pin_num];	
	
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = pre_pri;
